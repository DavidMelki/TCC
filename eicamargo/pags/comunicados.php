<?php include '../php/sessao.php'; ?>
<!DOCTYPE html>
<html lang="pt-BR">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Eicamargo - Comunicados</title>
    <!-- Ícones Bootstrap -->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">
     <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <link rel="stylesheet" href="../css/comunicados.css">
    <link rel="icon" type="image/icon" href="../css/img/logoeicamargo.png">
    <meta name="author" content="Alexandre Castello, David Melquiades, Bruna de Mello e Julia Akemi">
</head>

<body>

    <div class="app-container">

        <!-- ==================== COLUNA 1: MENU LATERAL ==================== -->
        <aside class="sidebar-left">
            <div class="logo">
                <img src="../css/img/logoeicamargo.png" alt="Logo">
            </div>

            <ul class="nav-menu">
                <li>
                    <a href="sugestoes.php" class="nav-item">
                        <i class="bi bi-lightbulb-fill"></i> Sugestões
                    </a>
                </li>
                <li>
                    <a href="vendas.php" class="nav-item">
                        <i class="fa-solid fa-cart-shopping"></i> Vendas
                    </a>
                </li>
                <li>
                    <a href="comunicados.php" class="nav-item active">
                        <i class="fa-solid fa-bullhorn"></i> Comunicados
                    </a>
                </li>
                <li>
                    <a href="mensagens.php" class="nav-item">
                        <i class="fa-regular fa-envelope"></i> Mensagens
                    </a>
                </li>
                <li>
                    <a href="curso.php" class="nav-item">
                        <i class="fa-solid fa-graduation-cap"></i> Curso
                    </a>
                </li>
                <li>
                    <a href="perfil.php" class="nav-item">
                        <i class="fa-regular fa-user"></i> Perfil
                    </a>
                </li>
            </ul>

            <div class="profile-footer">
                <div class="profile-info">
                    <div class="avatar" style="width: 40px; height: 40px; border-radius: 50%; background-image: url('../uploads/<?php echo !empty($usuarioLogado['foto_perfil']) ? htmlspecialchars($usuarioLogado['foto_perfil']) : 'default.png'; ?>'); background-size: cover; background-position: center;"></div>
                    <div class="dados">
                        <span class="nome"><?php echo htmlspecialchars($usuarioLogado['nome']); ?></span>
                        <span class="usuario">@<?php echo strtolower(str_replace(' ', '', $usuarioLogado['nome'])); ?></span>
                    </div>
                </div>
                <i class="bi bi-three-dots"></i>
            </div>
        </aside>

        <!-- ==================== COLUNA 2: FEED CENTRAL ==================== -->
        <main class="feed">

            <div class="top-feed">
                <h2>Comunicados Gerais</h2>
            </div>

            <!-- Posts / Lista de Comunicados -->
            <div class="lista-posts">

                <div class="post">
                    <div class="avatar-post"></div>
                    <div class="conteudo-post">
                        <div class="post-header">
                            <h4>Davi Afonso <i class=""></i></h4>   <!-- espaço para por o simbolo de verificado-->
                            <span>@AfonsoDavi · 2d</span>
                        </div>
                        <p>
                            A entrada e saída geral dos alunos será realizada exclusivamente pelas catracas digitais a partir de segunda-feira. Por favor, tragam suas carteirinhas.
                        </p>
                        <div class="post-acoes">
                            <button><i class="bi bi-heart"></i> 24</button>
                            <button><i class="bi bi-chat"></i> 5</button>
                            <button><i class="bi bi-share"></i></button>
                        </div>
                    </div>
                </div>

                <div class="post">
                    <div class="avatar-post"></div>
                    <div class="conteudo-post">
                        <div class="post-header">
                            <h4>Coordenação Pedagógica <i class=""></i></h4> <!-- espaço para por o simbolo de verificado-->
                            <span>@coordenacao · 4d</span>
                        </div>
                        <p>
                            Lembrete: As inscrições para os cursos de extensão e feira de ciências encerram nesta sexta-feira. Não deixem para a última hora!
                        </p>
                        <div class="post-acoes">
                            <button><i class="bi bi-heart"></i> 42</button>
                            <button><i class="bi bi-chat"></i> 12</button>
                            <button><i class="bi bi-share"></i></button>
                        </div>
                    </div>
                </div>

            </div>

        </main>

        <!-- ==================== COLUNA 3: DIREITA ==================== -->
        <aside class="right-bar">

            <div class="card-info">
                <h3>Avisos Importantes</h3>
                <p>Comunicados oficiais são postados apenas por administradores e coordenadores da instituição.</p>
            </div>

            <div class="links">
                <a href="#">Termos de Serviço</a>
                <a href="#">Política de Privacidade</a>
                <a href="#">Política de Cookies</a>
                <a href="#">Acessibilidade</a>
                <a href="#">Mais</a>
            </div>

            <p class="copyright">© 2026 Camargo aranha.</p>

        </aside>

    </div>

</body>

</html>